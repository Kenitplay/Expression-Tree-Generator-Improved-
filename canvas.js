...
(function () {
    ...
    document.getElementById('generate-tree').addEventListener('click', () => {
        ...
        if (expression) {  // Check if the expression is not empty
            expression = expression.replace(/\s+/g, '').toLowerCase(); // Normalize input
            var postfix = infixToPostfix(expression); // Convert infix to postfix
            if (postfix !== null) {
                // Display the postfix expression
                document.getElementById('postfix-output').value = postfix.join(' '); // Join array to string for display
